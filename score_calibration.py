...
--------------------------
1-5. (unchanged from v5 — see below)

6. FIX (v6) — params_data key mismatch resolved:
   v5 wrote calibration_params.json with keys "A" and "B".
   calibration.py._try_load_params() reads keys "platt_a" and "platt_b"
   (and also checks for a "method" key to confirm the method matches).
   This silent mismatch meant load_calibrator() never loaded the fitted
   params and always fell back to the unfitted calibrator, producing the
   "Calibrator not fitted — returning raw score" warning in run_validation.py.

   Fixed: params_data now uses the keys calibration.py expects:
     "method": "platt", "platt_a": A, "platt_b": B
   and also includes "n_samples" for the load log message.

7. FIX (v6) — dual write to cache dir:
   calibration.py._try_load_params() reads from:
     /tmp/drug_repurposing_cache/calibration_params.json
   but score_calibration.py was only writing to the project root.
   Now writes to BOTH locations so load_calibrator() finds the file
   automatically without any path plumbing in run_validation.py.

FIXES from v5 (preserved)
--------------------------
1. Circular dependency resolved — params written here, read by calibration.py.
2. Platt parameters refit from actual validation data.
3. ECE > 0.15 reported as known limitation, not hard FAIL.
4. Reads both 'test_cases'/'positive_results' key formats.
5. calibration_results.json includes paper_reporting_note.
